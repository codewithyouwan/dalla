...
export async function POST(req) {
  try {
    const { id_number, previous_skills_to_acquire, previous_interest_in_japanese_companies, user_prompt } = await req.json();
    if (!id_number) {
    ...
    // Construct payload for the prompt
    const payload = {
      interest_in_japanese_companies: data.interest_in_japanese_companies,
      aspects_to_learn: data.aspects_to_learn,
      previous_skills_to_acquire: previous_skills_to_acquire || 'なし',
      previous_interest_in_japanese_companies: previous_interest_in_japanese_companies || 'なし',
      user_prompt: user_prompt || 'なし',
    };

    // Generate prompt using the Prompt function
    const prompt = Prompt(payload, 'rethinkJapaneseCompany');
    ...
    console.error('Error in rethinkJapaneseCompany route:', error);
