...
export async function POST(request) {
  ...
  try {
    ...
    // If no internship data or all fields are empty/null, fetch project data
    if (
      error ||
      !data ||
      (!data.internship_1_title &&
        !data.internship_1_company &&
        !data.internship_1_period &&
        !data.internship_1_team_size &&
        !data.internship_1_technologies &&
        !data.internship_1_summary &&
        !data.internship_1_purpose &&
        !data.internship_1_role &&
        !data.internship_1_challenges &&
        !data.internship_1_outcome)
    ) {
      isInternship = false;
      ...
      if (error || !data) {
        console.error('Supabase error:', error?.message || 'No project data found', { id_number });
        return NextResponse.json({ error: 'No internship or project data found' }, { status: 404 });
      }
    }

    // Preprocess data (use internship or project fields)
    const experienceData = isInternship
      ? {
      ...
      : {
          internship_1_title: data.project_1_title || '',
          internship_1_company: '', // Projects don't have a company
          internship_1_period: data.project_1_period || '',
          internship_1_team_size: data.project_1_team_size || '',
          internship_1_technologies: data.project_1_technologies || '',
          internship_1_summary: data.project_1_summary || '',
          internship_1_purpose: data.project_1_purpose || '',
          internship_1_role: data.project_1_role || '',
          internship_1_challenges: data.project_1_challenges || '',
          internship_1_outcome: data.project_1_outcome || '',
        };

    // Generate prompt using Prompt function
    const prompt = Prompt(experienceData, 'internshipExperience');
      ...
      messages: [
        {
          role: "system",
          content:
            "Generate the response exactly as specified in the prompt. Include only the formatted output with no additional text, explanations, or deviations. Use ===FORM1-START=== and ===FORM1-END=== with exactly four lines of Japanese text, each starting with the specified labels.",
        },
    ...
    const result = {
      ...
      isInternship, // Include flag to indicate source
    };
