export default function Education({ education, handleArrayInputChange, addEducation, removeEducation, fetchEducation, isLoading }) {
  ...
  // Parse "University Name **[Major]**" into { name, major }
  const parseInstitution = (institution) => {
    const match = institution.match(/^(.*)\s*\*\*\[(.*?)\]\*\*$/);
    if (match) {
      return { name: match[1].trim(), major: match[2].trim() };
    }
    return { name: institution, major: '' };
  };

  // Rebuild the stored format from name + major
  const buildInstitution = (name, major) => {
    if (major) {
      return `${name} **[${major}]**`;
    }
    return name;
    ...
    <div className="mb-8">
      ...
      <div className='block border rounded-lg border-black p-2'>
        {sortedEducation.map((edu) => {
          const { name, major } = parseInstitution(edu.institution);
          const idx = edu.originalIndex; // use the ORIGINAL index

          return (
            <div key={idx} className="space-y-4 mb-4 pb-4 relative">
              {/* Year */}
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleArrayInputChange(
                      { target: { value: buildInstitution(e.target.value, major) } },
                      idx,
                      'institution',
                      'education'
                    )
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleArrayInputChange(
                      { target: { value: buildInstitution(name, e.target.value) } },
                      idx,
                      'institution',
                      'education'
                    )
              ...
              <div>
                ...
                <select
                  value={edu.degree}
                  onChange={(e) => handleArrayInputChange(e, idx, 'degree', 'education')}
                  className="mt-1 block text-black w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                >
